# 小说AI —— 基于 LLM 的中文长篇小说写作 Agent V1
...
v1 版本在已有能力基础上，重点补强了五个方向：大纲动态扩展/收缩、上下文检索与索引可靠性、连续性约束冲突治理、写作上下文可追溯性，以及 WebUI 诊断和自动化测试。具体分类如下：

### 已有功能增强
- **RAG / 混合检索**：原版本已明确支持「全文 & 语义搜索」和 bge-m3；v1 主要增强为关键词+向量混合召回、支持索引自动刷新、分片检索和一致性校验。
- **Idea 智能检索筛选**：原版本已说明按人物检索注入；v1 扩展为多通道召回、重排、故事线关联和章节适配判断。
- **上下文管理**：原版本已有「上下文压缩」和自动注入；v1 增加结构化上下文来源记录及 RAG 资料注入，保存每章生成时使用的资料、灵感、线程、约束和检索结果（provenance）
- **搜索索引**：不是新增搜索能力，而是增加索引指纹、过期检测、向量/文档数量校验和损坏保护。

### 全新功能
- **动态调整章节数**：新增 `chapters <project> <数量>` CLI 命令；WebUI 也增加「调整并续写大纲」功能。支持扩展后续章节、缩减未写章节，并保护已写内容。
- **Context Diagnostic 页面**：WebUI 新增只读诊断页，可查看章节生成时使用的灵感、线程、检索资料、上下文来源、约束、冲突和确认记录。
- **章节 Provenance 记录**：写作后新增 `chapters/provenance/*.json`，保存该章实际使用的上下文来源。
- **连续性冲突治理模块**：新增硬约束解析、冲突检测、冲突确认及治理状态持久化相关代码，属于对原版本中「连续性约束」的进一步实现。
- **评测与测试扩展**：新增 `evaluation/` 评测目录，以及相关测试文件。

### 总体结论
v1 相比 raw 不是重新增加整套小说创作功能，而是围绕原版本已有能力，重点补强了：
...
| `chapters <project> <数量>` | 调整总章节数；扩展时承接当前大纲生成后续章节，缩短时仅删除待写章节 |
...
项目同时提供 `evaluation/` 评测工具，以及覆盖阶段性功能的 phase1～phase6 测试。
