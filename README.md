# 小说AI —— 基于 LLM 的中文长篇小说写作 Agent V2
...
v2 版本在已有能力基础上，重点补强了五个方向：大纲动态扩展/收缩、上下文检索与索引可靠性、连续性约束冲突治理、写作上下文可追溯性，以及 WebUI 诊断和自动化测试。具体分类如下：

### 已有功能增强
- **RAG / 混合检索**：原版本已明确支持「全文 & 语义搜索」和 bge-m3；v2 主要增强为关键词+向量混合召回、支持索引自动刷新、分片检索和一致性校验。
- **Idea 智能检索筛选**：原版本已说明按人物检索注入；v2 扩展为多通道召回、重排、故事线关联和章节适配判断。
- **上下文管理**：原版本已有「上下文压缩」和自动注入；v2 增加结构化上下文来源记录及 RAG 资料注入，保存每章生成时使用的资料、灵感、线程、约束和检索结果（provenance）
...
### 总体结论
v2 相比 v1 不是重新增加整套小说创作功能，而是围绕原版本已有能力，重点补强了：
